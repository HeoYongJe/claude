...
- **나라 상세뷰(`.t-detail`, 랭킹 안에서 전환):** **detail은 흰 면만** — Surface(#f5f8ff) 틴트 전부 제거,
  ...
  · **Row1 `1fr 300px`**: 좌 "주요 품목 체감 물가"(흰 카드 — 라인아이콘(빅맥=hamburger·콜라=cup-soda·생수=droplet, `ITEM_SVG`, Primary)
    ...
    / 우 **파란 카드**: **대표(대형 44px) 숫자 = 환율 이득(변동, `.d-fxbig`)**, 그 아래 보조 텍스트로 "서울 대비 체감 절약/부담 N%"(`.d-savesub`, 작게)
    + 설명 + 물가 비쌈 시 흰 톤 **경고 라벨**(`.d-savecard__warn`) + 1,000원 환산·예상절약(추정) 행. **정상/경고 케이스 동일 위계**(둘 다 대표=환율이득).
    (2026-08-02: 이전엔 대표가 약한 '체감 절약%'라 "가장 이득" 메시지가 약했음 → 환율 이득으로 교체. 카드 내부 텍스트는 흰색 유지·방향은 ▲▼로.)
