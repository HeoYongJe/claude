...
- **나라 상세뷰(`.t-detail`, 랭킹 안에서 전환):**
  ...
  · 스탯 3: 전날 대비 환율 / 체감 물가 지수(부호 색) / **예상 절약 금액(추정)** — 절약률×70만원(1인7일 가정)이라 라벨에 "추정" 명시.
  · 좌 "주요 품목 체감 물가"(빅맥🍔/콜라🥤/생수💧 아이콘 + 서울 대비 막대·%, 실데이터) · 우 "추천 도시"(3, 큐레이션, 대표 지수).
  · 좌 "대표 먹거리"(3, **좌 아이콘/우 텍스트** 가로 정렬, 큐레이션+이모지) · 우 "여행 꿀팁"(3, 동일 레이아웃, cat→이모지 매핑).
  · 아이콘은 이모지(main.js `ITEM_ICONS`/`TIP_ICONS`, 먹거리는 `countryDetail.js`의 `foods[].icon`) — SVG/이미지로 교체 가능한 자리.
