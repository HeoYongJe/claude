...
- **나라 상세뷰(`.t-detail`, 랭킹 안에서 전환):** **섹션 배경은 body 그라데이션 그대로, 내용은 흰 카드 위주** — Surface(#f5f8ff) 틴트 전부 제거,
  카드는 `.dcard`(흰 배경+보더). **유일한 컬러 면 = 파란 절약 카드**.
  · **상단(head): 좌 그룹(`.detail__headL`) = 국기+국가명+순위 뱃지(중립색) 위에 "최근 한 달 대비 환율 ▲/▼%"(`.detail__fx`, 방향색) 좌측 정렬 /
  ...
  · **상세뷰 배경(2026-08-11 변경)**: 예전엔 `.rank.is-detail`이 `background:#fff`로 섹션 전체가 흰색이 됐으나(나라 클릭 시 배경색이 뚝 바뀌어 어색), **이제 섹션 배경은 body 그라데이션 그대로 두고 색 전환 없음** — 흰 면은 내용 카드(`.dcard`)·파란 절약카드만. 사용자 지시(PC에서 나라 클릭 시 배경색 변화 제거 + 모바일의 은은한 그라데이션을 PC에도 유지).
  · **상세뷰 상단 여백**: `.rank.is-detail { padding-top: 64px }`(리스트는 96px). 섹션 간격은 아래 섹션(rank)의 상단 패딩이 담당하고 **`.why`의 하단 패딩은 0**으로 둬 이중 여백(간격 불일치, 구 192px/128px)을 제거 → why→랭킹(리스트) ≈96 / why→상세 ≈64로 정리.
...
- **전체 랭킹 팝업(`.t-modal`):** "전체 랭킹 보기 →"(요약 띠)·"다른 나라 선택하기"(상세 head) 둘 다 이 팝업을 연다.
  **전체 국가를 결합 이득(환율+물가) 순으로** 나열(API `all`) — 각 행 = 순위 + 국기 + 이름 + [환율변동 뱃지(방향색) / 1,000원 환산]. **체감물가는 팝업에서 제거**(2026-08-16: 환율 바로 밑이라 '환율 대비'로 오해 — 물가는 상세뷰에서만). 환산은 국가명 밑(잘림)에서 **뱃지 밑 우측**으로 이동. **그리드는 1열**(환산이 길어 2열에선 국가명이 잘림).
...
- **인기 여행지(`.t-pop`, 랭킹 섹션 아래, 2026-08-16 재디자인 — 배경 티커 + 정지 타일 3개):** 결합 순위는 가성비 국가 위주라, 익숙한 여행지를 따로 보여줌. 섹션 배경 없음(body 그라데이션 위).
  ...
  · **정지 타일 3개(`.t-pop__tiles`, 클릭 대상)**: **뱃지 = 그 카드의 '주제(제목)'** — 국가 특징으로 오해되던 걸 막으려 **카드 최상단 eyebrow 위치 + 카테고리 라인 아이콘(`POP_ICON` 별/하강/경고) + 13px 강조**로 승격(국가명은 그 아래). 3종 = ①**대표 인기**(인기순 1위 — 조회수 데이터 없어 '가장 많이 봄'에서 정직하게 변경) ②**체감물가 최저**(cpiDiff 최소) ③**환율 이득·물가 부담**(환율 강세+물가 비쌈, `is-warn` 빨강 톤). 각 타일 = 국기+국가명+뱃지 / 1,000원 환산 / **체감물가 막대**(라벨 "체감물가" 13px — 추천 랭킹 `.rc-cut`과 문구·크기 통일. 왼→오, `width=|cpiDiff|` 100%상한, 저렴=파랑·비쌈=빨강, IntersectionObserver로 진입 시 채움) / 도시3 + "눌러서 상세". 클릭 → 기존 `openDetail` 재사용.
...
- 랭킹→상세뷰: 카드 클릭 → 랭킹 섹션 안에서 TOP3(`.t-rankview`)가 위로 사라지고 상세뷰(`.t-detail`)가 올라옴.
  **상세뷰 진입 시 섹션이 시간차로 순차 등장**(`.t-detail.is-in > *` nth-child transition-delay 0/.12/.24/.36s: head→품목·절약→도시·먹거리→꿀팁). 나라 전환 시 is-in 재적용으로 재생.
- **텍스트 위계(상세):** 섹션 타이틀 16 / 아이템 타이틀(음식·도시·팁·품목명) 15 / 본문·설명 13 / 메타(지수·라벨) 12 / 강조 수치 14~15·800.
- **추천 도시 hover:** 썸네일이 위로 살짝 뜨는 효과(`.d-city__thumb:hover translateY(-5px)` + 그림자)만. **이미지 확대(scale) 효과는 제거**(2026-08-11, 어색해서).
- **대표 먹거리:** 개별 박스 대신 **가로 구분선**(`.d-food + .d-food` border-top)으로 3항목 구분.
- **품목 물가 통화 라벨** 폭 80px로 확대(긴 국가명 잘림 방지, 막대는 축소).
...
- **실데이터:** 전날 환율(수출입은행 API) + 물가(KOTRA API, 국가 단위 빅맥/콜라/생수)를 서버가 계산해 캐시.
  ...
  **순위 산정(2026-08-16 변경 — 환율+물가 결합):** 예전엔 환율 변동률(`changePct`)만으로 정렬해 "환율은 좋지만 물가 비싼" 나라(튀르키예 등)가 계속 1위였음(문구 "함께 계산"과 모순).
  이제 **결합 이득 점수 `score = FX_WEIGHT·changePct + PRICE_WEIGHT·savePct`**(현재 1:1, `lib/ranking.js` 상수)로 정렬. changePct=원화 강세율(%), savePct=서울 대비 절약률(%, 쌀수록 +) — 둘 다 "원화 관점 이득 %"라 같은 방향으로 합산.
  탭 소속(강세/약세)은 환율 부호로 유지, **탭 안 순서·`all` 전체 랭킹은 결합 점수 내림차순**. 가중치는 상수로 조정 가능.
  **물가 데이터 있는 나라만 순위 집계(2026-08-16)** — 환율만으론 '이득'을 판단 못 해 제외(구: 물가 없는 나라를 0=중립으로 둬 '물가 미상'이 '물가 비쌈 확인된' 나라보다 위에 오는 불공정 제거).
  KOTRA(data.go.kr) 미수록 인기국 **홍콩·필리핀은 `lib/ranking.js`의 `MANUAL_PRICES`(USD 근사치)로 보강**해 포함, 나머지(노르웨이·헝가리·EU 등)는 랭킹에서 빠짐. 순위집계국 현재 25개국. (KOTRA는 67개국 고정 명단이라 홍콩·필리핀·헝가리·노르웨이·베트남 등은 원래 물가 데이터가 없음.)
  **API 응답:** `strong`/`weak`(top3) + **`all`(전체 국가 랭킹, 결합 이득 내림차순)** + **`counts`{tracked,strong,weak,avgSave}**
...
- **큐레이션 데이터:** 상세뷰의 **추천 도시·대표 먹거리·꿀팁은 API로 못 받는다**(KOTRA는 도시 단위 없음).
  ...
  (참고: "추적 중인 통화" 스탯 = `counts.tracked` = **순위 집계국 수(물가 데이터 있는 나라, 현재 25개국)**. 환율만 있는 나라는 랭킹·이 카운트에서 제외.)
  나라 추가 = 객체에 항목 하나 추가. 큐레이션 없는 나라 클릭 시 환율·물가는 실데이터로 보이고 도시/먹거리/꿀팁은 "준비 중".
  도시별 "물가 지수 %"는 실측이 아닌 **대표값**.
- **도시 이미지(자동 수집):** `tools/fetch-city-images-unsplash.js`가 Unsplash API로 각 도시 대표 이미지를 수집해
  `js/cityImages.js`(도시명→Unsplash URL) 생성(재실행 안전, 기존 유지·누락만 채움). 키는 `config.js`의 `UNSPLASH_ACCESS_KEY`.
  한글 도시명→영문 검색어는 도구의 `EN` 맵으로 관리(새 도시 추가 시 매핑 추가). **현재 90개 중 86개 수집**(나머지 4개는
  Unsplash 무료키 시간당 50회 한도로 밀림 — 리셋 후 재실행하면 채워짐). 없는 도시는 도시 아이콘 + '이미지 준비중' 폴백.
