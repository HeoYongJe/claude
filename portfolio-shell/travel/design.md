...
- **전체 랭킹 팝업(`.t-modal`):** "전체 랭킹 보기 →"(요약 띠)·"다른 나라 선택하기"(상세 head) 둘 다 이 팝업을 연다.
  **전체 국가를 결합 이득(환율+물가) 순으로** 나열(API `all`) — 각 행 = 순위 + 국기 + 이름 + [환율변동 뱃지(방향색) / 1,000원 환산]. **체감물가는 팝업에서 제거**(2026-08-16: 환율 바로 밑이라 '환율 대비'로 오해 — 물가는 상세뷰에서만). 환산은 국가명 밑(잘림)에서 **뱃지 밑 우측**으로 이동.
