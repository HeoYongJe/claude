...
## 7. Components (섹션별)
- **히어로:** 배경 `linear-gradient(120deg,#EBF1FC,#F4F8FF,#FCFDFF)` + 우측 은은한 블루 **도트 '지도' 배경**(`.hero__map`,
  radial 마스크로 페이드). 환율 카드: 흰 배경, radius 24, 상시 그림자, "오늘의 추천" + 변동률 뱃지(강세라 빨강)
  + 국가명(국기) + 1,000원 환산 + 설명. 부유 + 마우스 틸트. (실제 대륙 지도는 이미지 에셋 필요 — 현재는 도트 질감)
...
- **나라 상세뷰(`.t-detail`, 랭킹 안에서 전환):** **detail은 흰 면만** — Surface(#f5f8ff) 틴트 전부 제거,
  ...
  · **Row2 `1fr 1fr`**: 추천 도시(라인 리스트) | 대표 먹거리(라인 리스트, 번호 01/02/03).
