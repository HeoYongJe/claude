...
- **추천 도시 hover:** 카드 `translateY(-6px)` + 그림자, 썸네일 이미지 `scale(1.15)`(썸네일 overflow hidden).
...
- **B3 why 아이콘 드로잉**: 카드 `.is-revealed` 시 `path/circle[stroke]` dash 드로잉 + `rect` 페이드.
  **타이밍(2026-08-11): 섹션 도착 2s 후 1.2s에 걸쳐 천천히 그려짐**(`transition: stroke-dashoffset 1.2s … 2s`, rect는 2.4s 뒤 페이드) — 이전 .55s는 너무 빨라 조정.
