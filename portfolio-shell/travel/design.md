...
- **나라 상세뷰(`.t-detail`, 랭킹 안에서 전환):** **detail은 흰 면만** — Surface(#f5f8ff) 틴트 전부 제거,
  ...
  · **상단(head): 좌 국기+국가명+순위 뱃지(중립색) / 우 대표 숫자 "전날 대비 환율 변동"(`.detail__fx-val`, ▲/▼%, 방향색 크게)
    + "다른 나라 선택하기 →".** 순위 뱃지는 방향색 아님(중립 `#475569`/`#F1F5F9`).
  · **Row1 `1fr 300px`**: 좌 "주요 품목 체감 물가"(흰 카드 — 라인아이콘(빅맥=hamburger·콜라=cup-soda·생수=droplet, `ITEM_SVG`, Primary)
    + 이름 + [서울 회색막대/현지 파란막대 + 금액] + 큰 % 부호색)
    / 우 **파란 절약 카드**(서울 대비 체감 절약 % — **대표숫자를 head 환율변동에 넘기고 34px로 축소된 보조 지표** + 설명
    + 물가 비쌈 시 흰 톤 **경고 라벨**(`.d-savecard__warn`) + 1,000원 환산·환율변동·예상절약(추정) 행).
