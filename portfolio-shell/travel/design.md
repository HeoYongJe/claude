...
- **랭킹(흰 배경):** 흰 카드 **3개 균등 그리드**(gap 24, radius 20, 보더 `--border`). 세그먼트 토글(강세·유리 / 약세·불리,
  ...
  + "체감물가 ±xx%" pill(부호 색) + "상세 보기 →". **카드 클릭 → 상세뷰로 슬라이드 전환**(TOP3가 위로 사라지고 상세가 올라옴).
  · 변동률 화살표 = 강세율 부호(강세=▲/약세=▼), 숫자 절댓값. 요약 띠(Surface, 통계 3 + 버튼).
- **나라 상세뷰(`.t-detail`, 랭킹 안에서 전환):**
  · 상단: 국기 + 국가명 + 순위 뱃지 + "다른 나라 선택하기 →"(복귀).
  · **덩어리(면) + 내부 세로 라인 구분** — 개별 박스 금지. 공통 `.dbox`(Surface 면), 내부는 `border-left`로만 분할.
  · 스탯: **한 박스 3분할**(전날 환율 / 체감 물가 지수(부호색) / **예상 절약 금액(추정)**, 절약률×70만원 1인7일 가정 "추정" 명시).
  · **한 박스 가운데 세로 라인**: 좌 "주요 품목 체감 물가"(서울 vs 현지 **금액 비교 막대, 차오름 애니메이션**, %아님) / 우 "추천 도시"(라인 리스트).
  · 먹거리·꿀팁: **각 박스, 내부 3분할(세로 라인)**. 먹거리=**번호 01/02/03**(아이콘 대신), 꿀팁=**모던 라인 SVG 아이콘**(main.js `TIP_SVG`+`CAT_ICON` 매핑).
  · 막대 색: 서울=연회색, 현지=저렴 빨강/비쌈 회색. 금액은 원 표기.
  · 전환: `.t-rankview.is-out` ↔ `.t-detail.is-in`. 연타 방지 타이머 가드.
