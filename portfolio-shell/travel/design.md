...
- **전체 랭킹 팝업(`.t-modal`):** "전체 랭킹 보기 →"(요약 띠)·"다른 나라 선택하기"(상세 head) 둘 다 이 팝업을 연다.
  **전체 국가를 결합 이득(환율+물가) 순으로** 나열(API `all`) — 각 행 = 순위 + 국기 + 이름 + 1,000원 환산 + 환율변동 뱃지(방향색) + 체감물가.
  **모든 나라 클릭 → 해당 상세로 전환**(상세가 열려 있어도 `openDetail`이 내용만 교체). 오버레이·✕·Esc로 닫기.
  (API `all`/`counts` 없으면 강세+약세 top으로 폴백.)
- **인기 여행지 캐러셀(`.pop`, 랭킹 섹션 아래, 2026-08-16):** 결합 순위는 가성비 국가(인니·필리핀 등) 위주라, **자주 찾는 대중 여행국을 따로 바로가기**로 제공.
  고정 큐레이션 `POPULAR`(main.js): 일본·중국·미국·태국·싱가포르·스위스·유럽연합·베트남. (출국통계는 거의 안 바뀌어 실시간 연동 대신 고정 — 한국관광공사 데이터랩에 무료 API 있으나 실익 없어 미사용.)
  한 줄 **가로 스크롤 캐러셀**(chip: 국기+이름+환율뱃지+체감물가, 좌우 화살표 `.pop__arrow`, snap). 클릭 → 상세뷰.
  데이터 소스: 랭킹에 있으면 `ALLDATA`(순위 포함) → 없으면 **API `byCode`**(환율만 있는 유럽 등 전체 FX국 카드맵) → 그것도 없으면 최소객체(베트남 등, FX·물가 없음). `populateDetail`은 num/badge/rate 없으면 "인기 여행지"·"준비 중"·"환율 준비 중"으로 가드.
